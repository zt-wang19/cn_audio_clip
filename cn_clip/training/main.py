...
def main():
    ...
    audio_model_config_file = Path(__file__).parent.parent / f"clip/model_configs/{args.audio_model.replace('/', '-')}.json"
    print('Loading audio model config from', audio_model_config_file)
    assert os.path.exists(audio_model_config_file)
    
    text_model_config_file = Path(__file__).parent.parent / f"clip/model_configs/{args.text_model.replace('/', '-')}.json"
    print('Loading text model config from', text_model_config_file)
    assert os.path.exists(text_model_config_file)
    
    with open(audio_model_config_file, 'r') as fv, open(text_model_config_file, 'r') as ft:
        model_info = json.load(fv)
        for k, v in json.load(ft).items():
            model_info[k] = v
        model_info['pretrained'] = args.pretrained
